        ...
        <div class="mt-2 overflow-x-auto">
            <table
                ...
                <tbody>
                    ...
                    <tr v-else v-for="(tag, index) in leadTags" :key="tag"
                        ...
                        <td class="px-4 py-3 text-right w-[40%]">
                            <div class="flex items-end space-x-3.5">
                                <svg @click="editFilter(tag.id, tag.name)"
                                ...
                                <svg @click="deleteFilter('confirmation', tag.id)" xmlns="http://www.w3.org/2000/svg"
...
const filterType = ref('statusFilter');
...
const createTable = (page) => {
    ...
    let endpoint = `${import.meta.env.VITE_API_BASE_URL}status`;
    axios
        .get(endpoint, {
        ...
        .then((response) => {
            if (response.data.data.length > 0) {
                leadTags.value = response.data.data;
...
const deleteFilter = (type = "", filterId = "") => {
    ...
    let endPoint = `${import.meta.env.VITE_API_BASE_URL}status/${destroyFilterId.value}`;
...
const updateFilter = (index) => {
    ...
    let endpoint = `${import.meta.env.VITE_API_BASE_URL}status/${filterId.value}`;
    axios
        .put(endpoint, {
        ...
        .then((response) => {
            if (response.data.status) {
                ...
                leadTags.value[index].name = leadDetail;
